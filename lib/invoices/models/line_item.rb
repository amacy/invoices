class LineItem
  ...
  def find_by_invoice_number(invoice_number, *boolean)
    items = choose_db(*boolean).execute("select * from line_items where invoice_number = #{invoice_number}")
    items.map do |line|
  ...
  def save(*boolean)
    choose_db(*boolean).execute("INSERT INTO line_items (invoice_number, line_number, commit_date, commit_msg, hrs, rate, cost) 
                                VALUES (?, ?, ?, ?, ?, ?, ?)", 
